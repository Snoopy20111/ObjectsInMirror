...
func _on_trigger_train_audio_start_player_crossed() -> void:
	trainNode.activate_train_sound()

func _on_trigger_train_start_player_crossed() -> void:
	isTracking = true
	trainNode.activate_train() 
	emit_signal("trainStartTrack")

func _on_trigger_train_end_player_crossed() -> void:
	# Release the train to keep going past at its average speed so far
	isTracking = false
	trainNode.speed = _speedAvg

func _on_train_stop_crossing() -> void:
