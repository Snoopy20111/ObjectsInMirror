...
@export var engine_power:float = 900
...
func _integrate_forces(state):
	...
	get_input()
	apply_friction()
	calculate_steering(state.step)
	apply_central_impulse(acceleration)
	#linear_velocity += acceleration * state.step
	#move_and_slide()
...
func calculate_steering(delta):
	...
	var traction = traction_curve.sample(linear_velocity.x / engine_power)
	var dot_product = new_heading.dot(linear_velocity.normalized())
	
	if dot_product > 0:
		linear_velocity = linear_velocity.lerp(new_heading * linear_velocity.length(), traction)
	if dot_product < 0:
